--1.a
SELECT instructor.id AS instructor_id, instructor.name AS instructor_name, COUNT(DISTINCT teaches.course_id) AS distinct_courses_taught
FROM instructor
INNER JOIN teaches ON teaches.id = instructor.id
GROUP BY instructor.id
ORDER BY distinct_courses_taught DESC
LIMIT 3;
...
LEFT JOIN (SELECT course_id, year, semester, COUNT(id) AS registered
FROM takes
